...
# استدعاء الدالة مباشرة من ملف السكريبت الخاص بك
from import_mostaql import import_mostaql_projects 
...
class RefreshProjectsView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # ملاحظة: إذا كانت الدالة داخل import_mostaql.py لا تُرجع عدد المشاريع، 
            # يمكنك تعديل السكريبت ليُرجع الـ count.
            import_mostaql_projects()
            
            return Response({
                "message": "Projects refreshed successfully",
                "imported_count": "Check logs for details"
            }, status=status.HTTP_200_OK)
